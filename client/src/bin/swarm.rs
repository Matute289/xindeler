...
use vek::*;
use xindeler_client_core::{Client, ClientType, addr::ConnectionArgs};
...
fn run_client(
    ...
    opt: Opt,
    finished_init: Arc<AtomicU32>,
) -> Result<(), xindeler_client_core::Error> {
    let mut client = loop {
    ...
    let mut tick = |client: &mut Client| -> Result<(), xindeler_client_core::Error> {
        clock.tick();
