...
| EM-5.9 | **Main menu + connect flow** — menu, login, connecting/loading, credits, full server browser | 🟡 T56.29 (menu + first-run disclaimer + login + offline connect, real EM-4.2c handshake; first `AppState` menu↔gameplay machine; online transport deferred to T56.31) in PR; T56.30 (connecting/loading screen — real staged progress from the `ClientInitStage` handshake on a background boot thread + progress bar/spinner + server MOTD + data-driven tips/credits RON) in PR; T56.31 (server browser) pending | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
...
| EM-5.12 | **Settings + esc menu** — pause menu + all tabs | 🔵 **T56.38 + T56.39 in PR (`feat/bl82-em512-settings-esc-menu`, based on `development`, independent of the EM-5.9 #166→#168 chain).** Esc menu restructured from the old Resume+inline-graphics slice into a real pause menu (Resume / Settings / Controls / Servers-stub / Logout-stub / Quit=`AppExit`). New `settings_window.rs`: one `HudWindow::Settings` modal (F10) with a 9-tab bar. **Real tabs:** Interface (UI scale→`hud_scale`, crosshair→`combat_hud::Crosshair`), Video (full `GraphicsSettings`: tier presets + all toggles, tier→Custom on hand-edit, SSAO/TAA live via the moved `apply_graphics_settings`, rest persist for next boot), Controls (opens the existing EM-5.11 rebinding screen — not rebuilt), Gameplay (`CameraSettings` mouse-sensitivity + fly-speed), Chat (new `chat.opacity` wired live to the scrollback bg, closing chat.rs's own TODO). **Functional-but-simple:** Language (persists `XindelerSettings::language`; `en`-only until EM-5.16). **Honest stubs:** Sound (EM-5.10), Accessibility (EM-5.16), Networking (nothing configurable today). Struct extended with `interface`/`chat`/`language` (serde-default backward-compat, round-trip tested). All gates green (fmt, both clippy, engine-isolation, 321 client + 13 app tests). | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
...
| EM-5.14 | **Character select + creation** — char list + 3D preview + creation wizard | 🔵 **T56.32+T56.33 in PR (branch `feat/bl82-em514-character-select-creation`)** — new `NetCharList` broadcast mirror + `CharCreate/Delete/Select` client requests (`xindeler-protocol::charlist`), applied to the embedded player via new `EmbeddedPlayer` manual-selection stage + pass-throughs and a `CharListMirrorPlugin` bridge; pure-Bevy `char_select` screen (roster select/delete/create) + the 6-step wizard (Body→Appearance→Class→Alignment→Background→Finish, name always visible, valid defaults from step 1, Create only on the last step, per-class weapon filtering mirroring `valid_starter_items`) gated on the pre-existing `AppState::CharSelect`, reachable via a new `--char-select` launch flag; 3D preview reuses the EM-3.8x figure pipeline (`char_preview`, offscreen camera + `NetBody`/`NetLoadout` figure). Merged onto EM-5.9's now-merged `boot_eagerly`/menu-deferred-boot refactor (`ListenServerPlugin` gained both fields; `initial_state` centralized in `main.rs`). Verified: protocol replication round-trips + ignore-gated real-sim create-persists/delete round-trip tests + pure wizard-logic unit tests. Awaiting Matías's merge. | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
| EM-5.15 | **Crafting** — recipes/categories + full salvage/repair/modular-weapon tabs | 🔵 PR (T56.40-42) | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
