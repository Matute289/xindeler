...
| EM-5.3 | **Skillbar / hotbar + cooldowns** — real drag-to-assign (reorder/rebind hotbar slots via a genuine `xindeler_ui::slot` drag-drop, calling the embedded player's real `client::Client::change_ability`), real keybind labels sourced from EM-5.11's `xindeler-input` keymap (live-updating after a rebind), sim-driven slot count (whatever `ActiveAbilities::limit` reports today, `Some(5)` — NOT a hardcoded 10), cooldown greying + an inferred-total linear wipe + countdown text, M1/M2 read-only indicators. *New mirror:* `NetAbilities` (resolved ability-pool/slot-binding projection) + `NetCooldowns` (per-ability remaining) reading the sim's `ActiveAbilities`/`AbilityPool`/`AbilityCooldowns` (`xindeler-sim-bridge::hotbar::mirror_hotbar_state`) | ✅ **shipped** — `xindeler-protocol::hotbar` (`NetAbilities`/`NetHotbarSlot`/`NetAuxiliaryAbility`/`NetCooldowns`/`AssignHotbarSlot`/`LocalAssignHotbarSlot`) + `xindeler-sim-bridge::hotbar` (mirror + `apply_local_hotbar_assignment` calling `EmbeddedPlayer::assign_hotbar_slot`) + `xindeler-client::hotbar` (the screen); reuses the `xindeler_ui::slot` drag-drop primitive EM-5.6 shipped concurrently (PR #88) rather than a second competing implementation; 6 new client tests + 5 sim-bridge tests + 3 protocol tests, all passing; live-verified via `--listen-server --smoke-screenshot` |
