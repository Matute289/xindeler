...
| EM-5.20 | **Full es-LATAM (Spanish, Latin American) i18n coverage** — the multi-locale reactive pipeline (EM-5.16 T56.44) has existed since PR #176, but no locale besides `en` has ever had a real translation pass. There is a real open question this task must resolve first, not assume: `assets/voxygen/i18n/` already carries BOTH `es` (43→53-file gap vs. `en`'s catalogs; its own `_manifest.ron` declares it **"Español de España (Spanish - Spain)"**) AND `es-419` (43 files; its `_manifest.ron` already declares it **"Español de Hispanoamérica (Spanish - Latin America)"**, the correct BCP-47 tag for LATAM Spanish) — legacy Veloren locales, neither audited against the current `en` key set. The Bevy client's own language picker (`bevy/xindeler-client/src/settings_window.rs`'s `AVAILABLE_LANGUAGES`) only exposes `["en", "es"]` today — `es-419` isn't even selectable yet. **Decide first:** swap the picker to `es-419` (drop `es`), add `es-419` alongside `es` (both locales, two translation passes), or keep only `es` and translate it in neutral/LATAM-friendly Spanish instead of Iberian Spanish (contradicts its own manifest name). Once decided: audit every `.ftl` file under `assets/voxygen/i18n/en/` against the chosen target locale's directory, translate every missing/stale key, and keep both trees in sync going forward (every NEW `en` key added by any future task should get its translation in the same PR). Large content volume (every screen × every key) — expect a dedicated Haiku-content pass per `.ftl` file/domain, Sonnet wiring only if pluralization/grammar needs a Fluent select-expression restructure `en`'s doesn't have. Sequenced LAST in Phase 5 — after EM-5.16 item D (the Skill i18n table, the only other task still adding NEW English keys) ships, so this translates the settled key set rather than chasing a moving target; does not block on EM-5.13 itself (an unrelated play-session/QA gate). | ⚪ pending — spec/plan/tasks not yet written |
