...
| **5** | UI (bevy_ui + widget kit), audio & playable parity | 🔵 **in progress, close to done** — EM-5.1–5.12, 5.14, 5.15, 5.17, 5.18, 5.19 all ✅ shipped/merged (**EM-5.10 Audio now complete, all 5 sub-phases merged**). Only 2 things remain: **EM-5.16 (i18n)** the reactive pipeline is proven end-to-end but only 2 of ~15 screens are actually converted to real `.ftl` lookups (§Q6=A locks full coverage as required, not optional); **EM-5.13** the terminal cutover gate (Matías's own full-parity play session + sign-off), blocked until EM-5.16 is done. |
...
| EM-5.10 | **Audio** — direct in-house Kira integration (largest epic, splits 5.10a–e); music/SFX/ambience/spatial + full 252-file instrument bank | ✅ **all 5 sub-phases merged.** T56.34 (5.10a, PR #174) — `bevy/xindeler-audio` crate + Kira/cpal backend foundation. T56.35 (5.10b, PR #177) — SFX event mapping: movement/combat/campfire real off new `NetLocomotion`/`NetCombatMove` mirrors, `handle_outcome` partial (5/~40 `Outcome` variants), Block/Vehicle sub-mappers documented no-op stubs (no terrain-scenery/vehicle mirror exists yet). T56.36 (5.10c, PR #187) — music state machine (`soundtrack.ron`/`music_transition_manifest.ron`, biome/time/weather/site selection + weighted random + fades) + ambience loops (`ambience.ron`). T56.36b (5.10d, PR #192 + review follow-up commit `95a2a5928c`) — Kira spatial tracks + camera-anchored listener, real distance attenuation/stereo pan, underwater low-pass muffling (fixed the loud-constant-campfire regression). **T56.37 (5.10e) — the 252-file bard instrument note-bank, closing the epic:** the client-side `Music` sfx event `combat_sfx_mapper` explicitly deferred is now real — a new `NetInstrumentMove` mirror (`xindeler-sim-bridge::sfx::playing_instrument`, off `character_state.is_music()` + the equipped instrument's `ItemDesc::ability_spec()`) drives a new `music_sfx_mapper` (`xindeler-client::sfx`) that constructs `SfxEvent::Music(Instrument, ability_spec)` and lets the existing `trigger_sfx`/`sfx.ron` manifest (already carrying all 22 instruments' full note banks, ported from upstream) pick a random note per re-trigger. No new assets/manifest content needed — everything was already ported, only the "when to trigger + which random note" wiring was missing. | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
...
| EM-5.16 | **Accessibility, UI scaling & i18n depth** — subtitles, reduced-flashing, scaling, full multi-language i18n | 🔵 **T56.43 merged (PR #175)** — accessibility toggles (reduce-flashing dampens the low-health vignette, high-contrast `HudTheme` reconcile) + tutorial overlay; positional-sound subtitles stay deferred until EM-5.10c+ SFX-subtitle wiring happens (movement/combat/campfire SFX itself landed via 5.10b, PR #177, so this is now just a UI-wiring follow-up, not blocked on the audio backend). **T56.44 merged (PR #176)** — full multi-locale reactive Fluent pipeline (`CurrentLocale`+`LocalizedText`/`LocalizedLabel` hot-swap, `xindeler_ui::i18n::LocaleSyncSet`) proven end-to-end, but **only `settings_window.rs`+`esc_menu.rs` fully converted to real `.ftl` lookups** (`controls_screen.rs` heading only). **Remaining: convert every other screen** (diary/inventory/chat/crafting/trade/social/hotbar/combat_hud/map/char_select/menu/boss_nameplate/targeting) — §Q6=A locks this as REQUIRED full coverage, not optional, so this row isn't ✅ until that's done. | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
