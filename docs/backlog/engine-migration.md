...
| **3** | Voxel meshing, terrain & figures | 🔵 **in progress** — EM-3.1→3.8c done (PRs #7–#18): **real Xindeler terrain + entities + a controllable character + real animated `.vox` figures (quadruped/humanoid/birds) with a weapon now render in Bevy**; EM-3.9 (sprites+fluids) in progress; EM-3.8d/3.10/3.11 pending |
...
| EM-3.8d | Figure gear + polish — real equipped gear from inventory + lantern/back/glider + bird fly hysteresis + dedup nits | ⚪ |
| EM-3.9 | Sprites (grass/props) + fluids v1 (`river_velocity`) | 🔵 **NEXT** |
