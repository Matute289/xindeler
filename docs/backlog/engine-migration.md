...
| EM-5.10 | **Audio** — direct in-house Kira integration (largest epic, splits 5.10a–e); music/SFX/ambience/spatial + full 252-file instrument bank | 🔵 **T56.34 (5.10a — new `bevy/xindeler-audio` crate + Kira/cpal backend foundation: 4 mixer tracks + master, `.ogg`/`.wav` `AssetLoader`, pump system, volume `Res`) in PR** (`feat/bl82-em510a-audio-foundation`). Ports the old client's `voxygen/src/audio/` Kira-on-cpal architecture into a Bevy plugin (§Q2=B: direct integration, not `bevy_kira_audio`). 5.10b (SFX, T56.35) is next, dependent on this landing. | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
