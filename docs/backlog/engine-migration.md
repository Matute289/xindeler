...
| EM-3.11 **[M]** | In-game visual smoke (AO/TAA/fog/anims/perf vs old client) | 🔵 18 rounds so far, ALL merged (PRs #32/#34/#36/#37/#39/#40/#41/#43 + round 10 = PR #48, #63/#64/#65/#66/#67/#68/#69/#71 + round 18 = PR TBD): camera, tree/terrain color, far-mesh, perf, fog, ghost-hand/TAA, terrain black-frame, lighting, flicker, vsync, quadruped logging, diagonal stutter, NPC mirroring, distant-sprite shadow flicker, slope-descent camera flicker, background-disappears-for-1-2-frames, placeholder-box-vs-far-mesh seam artifact, tan/beige placeholder patch + distant-tree flicker, a beige frontier strip + tree-creation flicker — all fixed. **Round 18:** round 17's colour hint compared a placeholder's height against a SINGLE coarse point sample, which live instrumentation found rejected 98.6% of real chunks in ordinary terrain (not just caves); fixed with a local-neighbourhood min/max range check instead of another tolerance retune. Foliage flicker traced to a SEPARATE root cause: sprite spawn was decoupled from terrain-mesh readiness (a LIT sprite could appear over the terrain pipeline's unlit placeholder), unlike `xindeler-old`'s atomic mesh-worker response — fixed with a new `ChunkMeshIndex::has_real_terrain_mesh` gate. Not yet Matías-confirmed live (merged, awaiting his retest). **Full findings log:** `docs/design/specs/2026-07-09-bl82-em311-findings-log.md`. Open: EM-3.11p (diagonal-movement stutter, 5 rounds in, root cause still unidentified — needs a call on whether to keep going; Matías reported it much improved by round 10 but not formally closed). |
