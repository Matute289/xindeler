...
| **5** | UI (bevy_ui + widget kit), audio & playable parity | 🔵 **in progress, very close to done** — EM-5.1–5.9, 5.11, 5.12, 5.14, 5.15, 5.17, 5.18, 5.19 all ✅ shipped/merged, and **EM-5.16 (i18n)**'s required full-screen `.ftl` coverage is now also done (PR #189 converted every remaining screen; a few small documented follow-ups remain — positional-sound subtitles, a `Skill`/`BuffKind` name-lookup table — but these sit outside the original per-screen conversion gate). Only 2 things remain: **EM-5.10 (Audio)** 4 of 5 sub-phases merged (foundation, SFX, music/ambience, and spatial audio + muffling all shipped); only **EM-5.10e (instrument bank)** is left. **EM-5.13** the terminal cutover gate (Matías's own full-parity play session + sign-off), blocked until EM-5.10e is done. |
