...
| EM-5.17 | **HUD-D4 dark-gothic ARPG visual replacement** — replaces the current flat-`bevy_ui` HUD with Matías's own Notion-designed, PNG-asset-backed Diablo-IV-style HUD (resource orbs, ornate action bar, boss nameplate, party frames, rarity-tiered loot, "Path of Ascension" skill tree); also roots out 4 real input/logic bugs found in a live play session (chat can't hide, duplicate "Lv.1", top-left bar clutter, dead P/I hotkeys). 8-phase rollout, phases sequenced independently. | 🔵 **Phase 1 done** (this PR) — asset pipeline: 55 real PNGs copied to `assets/voxygen/element/ui/hud_d4/`, `HudImages`/`HudImageKey` lookup (53 wired variants), additive image-backed widget-kit variants (panel/orb-bar/slot/button), `UiMaterial` confirmed viable in Bevy 0.19 but v1 uses the simpler CPU-clip fill (shader scaffolded, unwired v2 on-ramp), shared `GlobalZIndex` scheme. 37 new tests. Phases 3/5/7/8+ gated on open design questions (§6 of the spec). | `specs/2026-07-15-bl82-em517-hud-redesign-design.md`, `plans/2026-07-15-bl82-em517-hud-redesign-plan.md`, `tasks/57-bl82-hud-redesign-tasks.md` |
