...
| EM-5.8 | **Social / group / dialogue** — player list, party frames, invites; NPC quest/dialogue (v1-minimal, AURORA seam). *New:* `NetPlayerList`/`NetGroupState`/`NetDialogue` (`xindeler-sim-bridge::social`) | ✅ **shipped** (PR pending review) — real `common::rtsim::Dialogue` (already-shipped AURORA seam, not a new schema); group actions/dialogue responses go through the embedded `Client`'s real network API (`send_invite`/`accept_invite`/`perform_dialogue`/…), never a direct sim write; 11+5+7 tests, verified live against a real booted listen-server (1200 frames, zero panics) — pixel-level HUD confirmation still needs an interactive session (same harness limitation EM-5.2 documented) |
