...
| EM-5.10 | **Audio** — direct in-house Kira integration (largest epic, splits 5.10a–e); music/SFX/ambience/spatial + full 252-file instrument bank | ✅ **T56.34 (5.10a)** merged (PR #174) — `bevy/xindeler-audio` crate + Kira/cpal backend foundation. 🔵 **T56.35 (5.10b — SFX event mapping) in PR** (`feat/bl82-em510b-sfx`). Ports the old client's `SfxEventMapper` sub-mappers as Bevy systems: movement (footsteps/swim/roll/sneak/climb/glide, off a new `NetLocomotion` mirror), combat (attack/wield/unwield, off a new `NetCombatMove` mirror), campfire (reuses existing `NetBody`/`Transform`, no new mirror) all REAL; a `handle_outcome` port over a new partial `NetOutcome` message (5 of ~40 `Outcome` variants: Explosion/Damage/Death/Block/PoiseChange — documented v1 cut). Block and vehicle sub-mappers are documented no-op stubs (no "blocks of interest" terrain-scenery mirror or vehicle/mount mirror exists anywhere yet — real follow-ups, not fabricated). New `bevy/xindeler-audio::sfx` module (SfxEvent/VoiceKind/SfxInventoryEvent ported verbatim from the frozen `sfx.ron` vocabulary + its `AssetLoader` + `trigger_sfx` playback). 5.10c (music/ambience, T56.36) is next. | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
...
| EM-5.16 | **Accessibility, UI scaling & i18n depth** — subtitles, reduced-flashing, scaling, full multi-language i18n | ✅ **T56.43 merged (PR #175)** — accessibility toggles (reduce-flashing dampens the low-health vignette, high-contrast `HudTheme` reconcile) + tutorial overlay; positional-sound subtitles stay deferred until EM-5.10b (SFX) lands. 🔵 **T56.44 in PR** (`feat/bl82-em516b-full-i18n`) — full multi-locale reactive Fluent pipeline: en-fallback-per-key `Localization::load`, `CurrentLocale` resource + `LocalizedText`/`LocalizedLabel` hot-swap chain (`xindeler_ui::i18n::LocaleSyncSet`), settings-aware bridge in `xindeler-client::localization`. `settings_window.rs` (dogfoods the Language tab) + `esc_menu.rs` fully converted to real `.ftl` lookups; `controls_screen.rs` heading converted (per-`GameInput` labels remain a follow-up). Rebased onto T56.43 post-merge (resolved the real `settings_window.rs` conflict + a duplicate `zlayer_audit` `SettingsWindowRoot` entry the rebase would otherwise have introduced). | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
