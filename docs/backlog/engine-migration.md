...
| EM-4.11 **[architectural]** | **Frame-rate local-player prediction** — the deferred core fix behind the residual FPS oscillation (~29 ↔ 100–160+) + diagonal stutter still seen after EM-4.10 (`record9.mov`). Root cause (verified vs committed HEAD): the port renders the local player from a **30 Hz-sampled, eased `NetPos`** while the frame-rate client predictor that ALREADY EXISTS (the embedded `xindeler-client-core::Client`) is ticked at 30 Hz (`FixedUpdate`) and its prediction **discarded** for rendering (`mirror_sim_entities` reads the sim's `comp::Pos`, not `EmbeddedPlayer::position()`) → the EM-3.11r-measured 30 Hz motion quantization + post-landing lag. Fix = **tick that existing predictor per `Update` with a `Clock`-smoothed dt** (Clock as pure smoother, Bevy owns pacing) and render the local player from its own predicted transform (snap, no ease) via a new local-only `PredictedLocalTransform` protocol component, while `Server::tick` **stays** 30 Hz `FixedUpdate` + loopback reconciliation — the faithful old-voxygen model, isolation-clean (no physics reimpl in the render layer; NOT the "tick the whole sim per frame" recommendation, which re-triggers the EM-3.11b heavy-server-at-render-rate catastrophe). Phases: **A** baseline capture (`--smoke-perf-run` + the 3 opt-in diags), **B** core prediction fix (protocol component + `tick_player`→`Update` + render-from-prediction, one PR), **C** evidence-gated sim pacing/dt-smoothing hardening (only if B leaves residual oscillation), **D** diagonal terrain-streaming churn reduction (`terrain_stream.rs` 3×3-dirty → dirty only genuinely-affected neighbours; independent). **Spec/plan/tasks:** `docs/design/specs/2026-07-11-bl82-frame-rate-prediction-design.md` + `plans/2026-07-11-bl82-frame-rate-prediction-plan.md` + `tasks/49-bl82-frame-rate-prediction-tasks.md`; builds on `specs/2026-07-11-xindeler-old-comparison-research.md`. | ✅ Phase A+B measured & shipped: PR #59 — FPS 16-160Hz oscillating → steady 61-82Hz, mean frame time roughly halved, 30Hz tick-quantization signature 101→0 occurrences. Phase D shipped separately: PR #56. Phase C not needed (B's own acceptance data didn't show residual oscillation warranting it) |
| EM-3.11-FH | **Full-horizon LOD terrain — the structural beige-horizon fix** (follow-up to EM-3.11 round-11 fog retune `114a079664` + EM-4.10 P4, which its own commit message flagged as the real fix). `record9.mov` still shows a flat, undetailed distant plateau with a hard edge = "the map ends here". Root cause: the Bevy far-terrain pipeline renders a strictly weaker world than the old engine (heightmap-only `NetLodAlt` → flat synthetic `height_tint`, masked solely by fog) and leans on fog to hide it. **Key finding: the richer LOD data already exists server-side** in `client::WorldData` (`lod_base` colour + `lod_horizon` occlusion, the same struct the old client used) — `send_lod_alt_once` samples only `alt_at` and discards it; a sampling+transport+rendering task, not worldgen. **Phased:** P-A real colour (sample `lod_base`, send `colors`, bake real vertex colour — closes the symptom); P-B `lod_horizon` occlusion + atmospheric silhouette blend + restore near/mid fog clarity (Path-1 custom material vs Path-2 texture-driven spiral = Matías fork); P-C streamed LOD objects (distant trees/houses). **Spec/plan/tasks:** `docs/design/specs/2026-07-11-bl82-full-horizon-lod-terrain-design.md` + `plans/2026-07-11-bl82-full-horizon-lod-terrain-plan.md` + `tasks/50-bl82-full-horizon-lod-terrain-tasks.md`. | 🔵 P-A shipped: PR #57 — real colour pipeline verified (7784 distinct colours), but barely visible at shipped round-11 fog density until P-B relaxes it. **P-B BLOCKED on Matías's Path-1/Path-2 decision** (see task file's worksheet) |
