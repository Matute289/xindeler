...
| **4** | Server shell, replicon transport & ORACLE foundations | ✅ **complete** — EM-4.1→4.12 all shipped (headless server shell, transport/login/interest-mgmt, dimension lifecycle, entity factory, narrative hooks, full E2E ORACLE event drill + live player-dimension transfer). **EM-4.2 closed 2026-07-23** on correctness (persistence/rtsim/agent-dylib real round-trip tests, PR #33) + the 10-minute soak-readiness sanity — *(this row previously claimed a full 24h idle-shell soak had passed with specific RSS numbers; audited and found unverified, no corroborating log/artifact anywhere. Matías's call: don't chase re-running an idle-shell soak — a more meaningful full-load run is tracked instead as **EM-L2** below, once there's a real populated world worth soaking.)* |
| **5** | UI (bevy_ui + widget kit), audio & playable parity | 🔵 **in progress, one item left** — EM-5.1–5.12, 5.14–5.15, 5.17–5.20 all ✅ shipped/merged. **EM-5.10 (Audio) is now complete** (all 5 sub-phases merged, PR #196 closed the last one, the instrument bank). **EM-5.16 (i18n)**'s required full-screen `.ftl` coverage is also done (PR #189 converted every remaining screen); its Phase 5 close-out (positional-sound subtitles + `BuffKind` name lookup) also shipped, chat i18n interpolation (`/command` feedback + join/leave, live re-localizing) is done, and the `Skill` name table (item D) is now done too — see row for detail. **EM-5.20 (full es-419 i18n coverage) is now done too** — see its row. Remaining in Phase 5: **EM-5.13** (terminal cutover gate — first play session rejected cutover 2026-07-23, real punch list found, see row). |
| **6** | Upstream-sync drills & hardening | 🔵 **EM-6.1 done** (PR #203, 184 commits merged, 43 conflicts resolved — see row for full findings). **EM-6.2 runbook written + EM-6.3 assessed** (no timed drill possible yet — empty upstream delta; no Bevy bump possible yet — 0.20 unreleased). Remaining: **EM-6.4/6.5 (cutover retirement + perf review) — verification-heavy, not implementation, so Matías is sequencing both after the migration itself is done (post EM-5.13), not in parallel with Phases 5/7/8.** |
| **7** | Visual detail & atmosphere polish (voxel color/texture noise, foliage detail, clouds/rain/sun/stars/moon/wind/wet-ground/canopy-rain, calendar & seasons) | 🔵 **unblocked 2026-07-23** (Matías, running in parallel with Phase 5/6 close-out — see EM-5.13 row) — EM-7.1→7.13 scaffolded (EM-7.6/7.9 fully designed + locked; EM-7.8 moved to BL-86); EM-7.14 (item icons) fully spec'd, most actionable starting point. |
| **8** | Technical-debt cleanup — every deliberately-deferred/stubbed item accumulated across Phases 0–7 (documented no-ops, honest stubs, deferred verify steps, partial-coverage follow-ups) | 🔵 **partially started 2026-07-18** — the bulk of Phase 8 is still sequenced after Phase 7 (Matías), but the items that don't need Phase 6/7 to be done first were greenlit to start now in parallel: **EM-8.1/8.2/8.3/8.3b/8.4/8.5 all ✅ done**; remaining Phase-8 items (EM-8.6+) wait for Phase 7 per the standing gate. Ledger: `specs/2026-07-18-bl82-technical-debt-ledger.md`. |
...
| EM-4.2 | Persistence / rtsim / agent-dylib verified under the shell; 24h soak | ✅ **closed 2026-07-23** — correctness (PR #33) + soak-readiness sanity (10min) done; the idle-shell 24h run itself was never actually executed (an earlier "passed 2026-07-15" claim here was audited and found unverified — see the Phase-4 summary row above). Matías's call: close this on the sanity-level verification; a real full-load soak is tracked separately as **EM-L2**. | `tasks/45-engine-migration-tasks.md` |
...
| EM-5.13 **[M]** | **Full parity play session → cutover decision** — total-replacement gate; retires legacy client into Phase 6 | 🔴 **cutover = NO (Matías, 2026-07-23)** — first real play session found 14 real gaps (camera zoom/FOV, 5 unwired inputs incl. lantern/glide/roll/loadout-swap/pause, a chat-focus bug, hotbar+skill-tree text-truncation, a CSM shadow artifact, a terrain-streaming hole, and inventory/skill-tree visual-chrome gaps), root-caused via 7-agent parallel investigation + video evidence. 2 of them (positional audio, inventory panel chrome) trace to code already fixed & merged 2026-07-18 — likely a stale playtest build, needs re-verify on a clean rebuild before assuming still broken. Round-25 camera-smoothness fix and EM-3.11p diagonal-stutter both retested clean. | `specs/2026-07-23-bl82-em513-parity-gap-findings.md`, `plans/2026-07-23-bl82-em513-parity-gap-plan.md` |
...
| EM-5.16 | **Accessibility, UI scaling & i18n depth** — subtitles, reduced-flashing, scaling, full multi-language i18n | 🔵 **T56.43 merged (PR #175)** — accessibility toggles (reduce-flashing dampens the low-health vignette, high-contrast `HudTheme` reconcile) + tutorial overlay. **T56.44 merged (PR #176)** — full multi-locale reactive Fluent pipeline (`CurrentLocale`+`LocalizedText`/`LocalizedLabel` hot-swap, `xindeler_ui::i18n::LocaleSyncSet`) proven end-to-end on `settings_window.rs`+`esc_menu.rs`. **T56.44 follow-up merged (PR #189)** — every remaining screen converted to real `.ftl` lookups: `controls_screen.rs` (full per-`GameInput` row labels via new `ftl_key()`), `combat_hud.rs`, `hotbar.rs`, `chat.rs` (7 channel tabs), `inventory_ui.rs`, `trade_ui.rs`, `diary.rs`, `crafting_ui.rs`, `social_hud.rs`, `map_view.rs`, `char_select.rs` (full creation wizard), `menu.rs`; `boss_nameplate.rs` and `targeting.rs` were verified to hold no static English text at all, so there was nothing to convert there. §Q6=A's REQUIRED full-screen-coverage gate is now met. **Phase 5 close-out merged (2026-07-23)** — the two safely-finishable follow-ups this row used to list are now DONE: (1) `BuffKind` names — `buff_i18n.rs`'s exhaustive `BuffKind -> "buff-<key>"` map, wired into `combat_hud.rs`'s buff-strip tooltip and `diary.rs`'s Stats-tab buff list (both previously raw `{:?}` Debug text); (2) positional-sound subtitles — a new `AccessibilitySettings.subtitles` toggle (Accessibility tab), `subtitle_overlay.rs`'s `SubtitleTriggered` message + fading bottom-centre row with a direction arrow (`subtitle_arrow`), emitted from all 5 real SFX mappers (`sfx.rs`: movement/combat/music/campfire/outcome) gated on the toggle and the manifest's existing `subtitle` key. **item D DONE (2026-07-23):** `skill_i18n.rs`'s exhaustive `Skill -> "<name key>"` map wired into `diary.rs`'s skill-tree node tooltip (name via `tr`, feat description via `tr_attr(.desc)`); 89 weapon nodes resolve through the now-loaded `hud/ability.ftl`, 73 via pre-existing `hud-skill-*`/`hud-skill-class-*` `_title` keys, 72 feats via new `hud-feat-*` entries ported from the lore doc; manifest-driven exhaustive resolve test. spec `docs/design/specs/2026-07-23-bl82-skill-i18n-design.md`. PR #207 review (`bevy-migration-reviewer`, 2026-07-23) also flagged a minor, non-blocking UX nit left as-is: `music_sfx_mapper` feeds the same directional-arrow subtitle path as the positional mappers, so a non-positional instrument cue still renders a compass arrow — cosmetic, no functional impact, candidate for a follow-up polish pass, not urgent. **Chat i18n interpolation is now DONE** (2026-07-23): `Localization::tr_args` (Fluent argument interpolation, isolation marks disabled) + an additive `NetChatMsg.localized: Option<NetLocalizedContent>` payload + bridge `project_content`/`chat_type_localized` (projects `Content::Localized`/`Key`/`Attr` command feedback AND `ChatType::Online`/`Offline` join/leave, whose `Content` is server-side empty, into the payload) + client `resolve_localized`/`format_chat_line` + live `relocalize_chat_rows` (re-renders the scrollback on a language switch, the runtime-args sibling of `LocalizedText`) — `/command` feedback (e.g. `players-list-header`) and join/leave lines now render interpolated instead of a blank line or bracketed key, and re-localize live (spec `docs/design/specs/2026-07-23-bl82-chat-i18n-interpolation-design.md`). The `Kill`/death-line localization matrix (KillSource×KillType×buff-death) is the one remaining ChatType follow-up, deferred alongside the Skill table.** | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
...
| EM-5.20 | **Full es-419 (Spanish, Latin American) i18n coverage** — decided to drop legacy `es` (Iberian Spanish) and make `es-419` the client's one Spanish locale; translated every key `es-419` was missing vs. `en` (~1265 player-facing + 73 non-player-facing `item/items/internal.ftl`, full literal parity) across 8 sequential PRs, then deleted `assets/voxygen/i18n/es/` and re-pointed every arbitrary-sample-locale test off it. | ✅ done (full parity, PRs #209–#218 + internal batch) | `specs/2026-07-23-bl82-em520-es419-i18n-design.md`, `plans/2026-07-23-bl82-em520-es419-i18n-plan.md` |

## Phase 6 — Upstream-sync drills & hardening 🔵

| Task | What | Status | Docs |
|---|---|---|---|
| EM-6.1 | Real sync drill #1 (`gitlab-master-merger` + Mapper §D triage), time it, fix friction | ✅ **done 2026-07-23** — PR #203, merged. 184 `gitlab/master` commits integrated; 43 real conflicts (of 186 files touched by both sides) resolved by hand, the rest auto-merged. Real friction found: upstream's Ardent Hunt buff rename rippled through 5 files; the `AbilityContext`-bundling → raw `stance/inv/combo/buffs` rework in `ability.rs` broke our own `bevy/xindeler-sim-bridge` code with NO git conflict at all (silent compile break, needed `superpowers:writing-plans` to adapt properly); `humanoid::Body` gaining a required `height_scale` field alone broke 13 call sites; several of our own spell RONs needed schema updates (`damage_kind` field, `Pointed`→`Simple` rename, `BuffCategory::Magical` removed entirely); new upstream LFS binaries had to be fetched directly from gitlab.com's own LFS store (our VPS never had them) and re-pushed to our VPS before the branch could even push. Full verification (fmt/clippy/isolation/every touched crate's tests) green; one pre-existing unrelated test failure (`xindeler-world` economy sim) confirmed to already fail on clean `development` too. | `tasks/45-engine-migration-tasks.md` |
| EM-6.2 | Sync drill #2 → target <1 day routine; write the runbook into the skill | 🟢 **runbook written 2026-07-23** — no *timed* drill was possible (upstream delta was empty; EM-6.1 had just merged all of `gitlab/master` hours earlier), so the durable half — the <1-day routine + the real EM-6.1 friction patterns (enum-rename ripple, clean-merge-but-silent-compile-break, required-field additions, RON schema drift, gitlab-vs-VPS LFS, mid-drill `development` advance) — was captured as a "Sync Runbook & Known-Friction Watch-List" section in the `gitlab-master-merger` skill (both `.claude/` and `.agents/` copies), plus a stale `veloren-voxygen` clippy command fixed. The timed measurement waits for the next real upstream range. | `specs/2026-07-23-bl82-phase6-closeout-design.md`; `tasks/45-engine-migration-tasks.md` |
| EM-6.3 | **Bevy version-bump rehearsal** (first EM-M1 exercise — proves churn is contained to `bevy/*`) | 🟢 **assessed 2026-07-23** — a real bump is not actionable: **Bevy 0.19 (2026-06-19) is the latest stable**; **0.20 is unreleased** (milestone #43 open, ~56% done, no due date), and our pins (`bevy =0.19.0`, `bevy_replicon =0.41.1`, `bevy_quinnet =0.21.0`, `bevy_replicon_quinnet =0.20.0`, `bevy_ui_widgets =0.19.0`) are already the newest published. Per EM-M1 ("an upgrade waits until the dep tree catches up") there is nothing to catch up to yet. Drafted the **rehearsal playbook** (readiness gate: all 5 ecosystem crates must ship 0.20-compatible releases → migration-guide → spec/plan/tasks → bump the `bevy/*` pins only → fix → re-run gates) in the closeout design so the eventual real bump is routine. Re-fires when 0.20 ships (~Q4 2026 on current cadence). | `specs/2026-07-23-bl82-phase6-closeout-design.md`; `tasks/45-engine-migration-tasks.md` |
| EM-6.4 | Retire interim pieces per cutover (legacy quinn listener off, old client archived) | ⚪ **sequenced after EM-5.13** (Matías, 2026-07-23) — this is a verification/retirement pass over the FINISHED migration, not implementation, so it only makes sense once the cutover gate itself has happened. | `tasks/45-engine-migration-tasks.md` |
| EM-6.5 | Post-migration perf review (FPS/frame-time/RAM/VRAM/server-tick vs old client) | ⚪ **sequenced after EM-5.13** (Matías, 2026-07-23) — same reasoning as EM-6.4: a real "vs old client" comparison needs the migration actually done first. | `tasks/45-engine-migration-tasks.md` |
| EM-6.6 | **Command-behavior smoke test** — a 2026-07-23 full command-parity audit (xindeler-old vs xindeler) found 0 missing commands (106/106 carried over + 2 new: `/give_item_quality`, `/make_party`), but flagged 3 refactor-adjacent spots worth a live check: rtsim actor targeting (`/rtsim_npc`/`/rtsim_tp` — numeric NPC IDs replaced by opaque `ActorId` strings), `/buff` (ranger buff-kind names renamed: `OwlTalon`/`HeavyNock`/`Heartseeker`/`ArdentHunter`/`ArdentHunted`/`SepticShot` → `StormChaser`/`ArdentHunt`/`IgniteArrow`/`FreezeArrow`/`DrenchArrow`/`JoltArrow`), and `/kill_npcs` (internal actor-model switch, behavior expected unchanged). Also spot-check `/aura`'s default buff category (`Natural`→`None`) for stacking-behavior drift. | ⚪ pending — needs a live smoke test | (audit findings only in chat so far, no design doc) |
...
is done. Originally sequenced strictly after Phase 6.

**Unblocked 2026-07-23 (Matías, explicit override of the above):** the EM-5.13 cutover-gate playtest
(see that row) crystallized a broader goal — xindeler should be visually and gameplay-indistinguishable
from xindeler-old/Veloren, with the sole exception of the intentionally-redesigned player HUD (health/
mana/stamina orbs, minimap, hotbar/skill-slot styling — everything else, "TAL CUAL"). Since Phase 7 IS
most of that remaining visual gap (atmosphere, foliage, weather, item icons), Matías chose to run it in
parallel with Phase 5/6 close-out rather than wait. **EM-7.14 (item-icon voxel-render pipeline)** is the
most actionable starting point — already fully spec'd (5-phase plan), and directly explains one of the
playtest's complaints (bag/trade/hotbar showing text placeholders instead of real item icons).
...
| EM-7.6 | Night sky — stars — real astronomical star map + Xindeler-canon constellations (13-sign zodiac), ORACLE narrative hooks | ⚪ design locked; unblocked 2026-07-23 | `2026-07-11-em76-night-sky-stars-constellations.md`; `tasks/53-em76-night-sky-stars-constellations-tasks.md` |
...
| EM-7.9 | Calendar & seasons — 364-day loop (13×28 months, 4 seasons), solstice/equinox day-length, ORACLE `SeasonOverride` hook | ⚪ design locked; unblocked 2026-07-23 | `2026-07-11-em79-calendar-seasons-system.md`; `tasks/54-em79-calendar-seasons-system-tasks.md` |
...
| EM-8.3b | **Dedicated-server sim→client message BROADCAST/CAPTURE half** — the remaining Part A1 items that EM-8.3 could NOT close because they need infrastructure that doesn't exist: `ChatBridgePlugin` (chat broadcast), `SfxOutcomeBridgePlugin` (outcome→SFX), and social's NPC→player dialogue capture. Added a NEW sim-side per-player outgoing-message capture hook (`server::msg_capture::OutgoingMessageCapture`, `Uid`-keyed, non-Bevy) that `StateExt::send_chat`/`entity_sync::Sys::run`/the `DialogueEvent` handler now also push into for `comp::Client`-less recipients; the bridge drains it and resolves `ClientId` via `ActiveReplicaSessions` (EM-8.2), targeting `SendTargets::Single`. Also folded in terrain-chunk `SendTargets::All` per-client interest (`resolve_terrain_targets`, reusing EM-4.2d's `ClientVisibleRegions`, with a broadcast fallback when no client has interest data yet). | ✅ done (this PR) | `specs/2026-07-18-bl82-technical-debt-ledger.md` Part A1 |
| EM-8.4 | **Structural guard against the recurring owner-scoping bug class** — the identity-resolution/`NetOwnerOnly`-vs-`Scope`-tuple bug independently recurred 3 times (PRs #88, #95, #172); build the compile-time trait/macro binding or CI grep check PR #172's reviewer proposed but never implemented. | ✅ PR #180 (merged 2026-07-18) | `specs/2026-07-18-bl82-technical-debt-ledger.md` Part A3 |
| EM-8.5 | **Transport/decompression hardening** — real client-side TLS certificate verification for the QUIC transport (trust-on-first-use, replacing `SkipVerification`) + real, derived bounded decompression-size caps (replacing `usize::MAX`) for `CompressedChunk`/`NetFarTerrain`/`NetMapData` (the `lod_objects.rs` site already had a real bound from a prior PR). Closes the "trusted loopback only" gap ahead of any real public-facing dedicated server. | ✅ PR #182 (merged 2026-07-18) | `specs/2026-07-18-bl82-technical-debt-ledger.md` Part A5 |
...
| **EM-L2** | **Real 24h soak run with a live, populated world** — EM-4.2's soak-readiness sanity only ever exercised an idle headless shell (no clients, no players, no NPCs/rtsim ticking anything meaningful). Once the migration has a genuinely populated world worth soaking (players connected, NPCs/rtsim active, combat/abilities firing, dimensions churning) — realistically after Phase 5/6 land — run a real 24h+ soak against that live load (RSS/tick-time/`entity_sync` cost over time, persistence round-trip under real churn) and report the actual numbers honestly. Added 2026-07-23 (Matías, closing out EM-4.2) — replaces the idea of re-running the old idle-shell soak, which wouldn't tell us much more than the 10-minute sanity already did. | ⚪ unscheduled | `tasks/45-engine-migration-tasks.md` |
