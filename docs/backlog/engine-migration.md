...
| EM-3.12 | **Third-person camera collision (spring-arm)** — stop the camera clipping through solid geometry (Matías: looking up from below, the eye keeps its fixed 9 m boom and passes through the floor → "todo se vuelve transparente"; same for trees/walls/mountains). v1 = spring-arm pull-in only (the exact reported bug **and** full parity with `xindeler-old`, which ships only a spring-arm). **Key finding:** the raycast stays entirely in the pure-Bevy `xindeler-client` crate — the client already holds decoded terrain blocks (`terrain_stream.rs` `SharedTerrain`, already uses `common::vol::ReadVol`+`VolGrid2d`), so it reuses the same voxel DDA ray the physics uses (`phys/collision.rs:548`, `is_solid()`) with no sim round-trip and no new isolation surface; one cast covers terrain+mountains+walls+trees (trees are `Wood`/`Leaves` blocks). Composes with the EM-4.11 eased focus by clamping the *arm length* from the already-eased *pivot* (snap-in / ease-out), applied as the final step (mirrors old voxygen `Camera::update`→`compute_dependents`). Occlusion-fade (Phase 2) + over-the-shoulder (Phase 3) deferred. **Spec/plan/tasks:** `docs/design/specs/2026-07-11-bl82-camera-collision-design.md` + `plans/2026-07-11-bl82-camera-collision-plan.md` + `tasks/52-bl82-camera-collision-tasks.md`. | ⚪ spec/plan/tasks authored (Opus); awaiting Sonnet dispatch. Reviewers: bevy-migration + rust-perf. `XINDELER_CAMERA_COLLISION=0/1` kill-switch for Matías's live A/B (incl. `is_solid` vs `is_opaque` leaf-canopy call). |
| EM-3.11 **[M]** | In-game visual smoke (AO/TAA/fog/anims/perf vs old client) | 🔵 16 rounds so far (PRs #32/#34/#36/#37/#39/#40/#41/#43 + round 10 = PR #48, open): camera, tree/terrain color, far-mesh, perf, fog, ghost-hand/TAA, terrain black-frame, lighting, flicker, vsync, quadruped logging, diagonal stutter, NPC mirroring (fixed), distant-sprite shadow flicker (fixed), slope-descent camera flicker (fixed, round 13), background-disappears-for-1-2-frames (fixed, round 14 — a fogged placeholder-chunk box, not the far mesh), placeholder-box-vs-far-mesh seam artifact (fixed, round 15 — a `bevy-migration-reviewer` hypothesis checked live and confirmed: round 14's fog-immune placeholder read as a stark flat box against the far mesh's own haze; fixed with a capped, host-installed `PlaceholderHazeTint` blend), tan/beige placeholder patch + distant-tree flicker (fixed, round 16 — SAME placeholder box, not a new bug or a fog-colour/time-of-day defect: live-sampled pixels were genuinely near-neutral, matching round 15's own measurement, but read as warm tan/beige by simultaneous colour contrast against the cooler far-mesh/sky around it; `PLACEHOLDER_HAZE_BLEND` retuned 0.2→0.5, still capped so it can never fully wash out) — none yet Matías-confirmed live. **Full findings log:** `docs/design/specs/2026-07-09-bl82-em311-findings-log.md`. Open: EM-3.11p (diagonal-movement stutter, 5 rounds in, root cause still unidentified — needs a call on whether to keep going). |
| EM-3.11 full-horizon LOD terrain — Phase A (real colour) | Structural follow-up to round 11's fog-masking stopgap: the embedded `Client`'s `WorldData.lod_base` (real per-chunk colour) already existed server-side but was never sampled/sent — the far mesh baked a synthetic 2-stop height gradient instead. Spec/plan/tasks: `docs/design/specs/2026-07-11-bl82-full-horizon-lod-terrain-design.md` + companion plan/tasks-50. | ✅ Phase A — `client::WorldData::col_at` + `NetLodAlt`→`NetFarTerrain` (adds an index-aligned `colors` blob + a reserved-empty `horizon` blob for Phase B) + server-side `col_at` sampling + client bakes real per-cell vertex colour (`cell_color`, retiring the synthetic `height_tint`); `HAZE_BLEND` 0.35→`FAR_HAZE_BLEND` 0.12. Verified via a real-embedded-world integration test + a `--smoke-screenshot`/`XINDELER_SMOKE_FAR_MESH_CAM=1` visual check (real varied colour confirmed present and index-aligned). **Finding for Phase B:** at the shipped round-11 `fog_density` the far mesh is ~99.9% fog-opaque at its nearest visible point by design, so the real-colour improvement is barely perceptible live until Phase B relaxes fog — Phase A's data/render pipeline is complete and correct, but alone may not fully read as "fixed" in free-roam until Phase B lands. Phase B (horizon occlusion + atmospheric silhouette + fog-density restore) and Phase C (streamed LOD objects) remain ⚪, gated on Matías's Path-1/Path-2 call for Phase B. |
...
| EM-4.2b | Transport backend spike — `ReplicaTransport` abstraction + `quinnet` impl (real network, not loopback); renet2 evaluated, not adopted (lags our Bevy/replicon pins) | 🔵 PR #46 (folded in), reviewed clean, real dual-transport acceptance test passing |
| EM-4.2c | Login / session handshake bridged to the sim's accounts + persistence | 🔵 PR #46 (folded in), reviewed clean, real login-handshake acceptance test passing (offline + online-mode paths, duplicate-login kick) |
| EM-4.2d | Interest management — per-client visibility (region/distance + `DimensionId`); bandwidth vs old protocol | 🔵 PR #46 (folded in), reviewed clean, ~2x measured bandwidth improvement (party-scale scene) |
...
| EM-7.6 | Night sky — stars + constellations — v1 = a **real** astronomical star map (real RA/Dec/magnitude/colour as the scientific base) with **one** Xindeler-canon constellation layer drawn onto it (deity-tied figures + a 12-sign zodiac, anchored on Yssira the star-keeper + Velora's Wheel of Years — NOT the real IAU set, which is research grounding only), positioned by an EM-7.9-compatible sky clock, plus a clean empty `SkyOverride` hook for ORACLE to later mutate the sky (star goes dark / new sign / comet). Data-driven RON, no new dep, builds the shared SkyRig skeleton. Detailed design: `specs/2026-07-11-em76-night-sky-stars-constellations.md` (one constellation-naming worksheet open; also flags a 12-sign-zodiac vs 13-month seam with EM-7.9, see its Q-ZOD) | ⚪ |
