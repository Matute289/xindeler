...
| EM-5.9 | **Main menu + connect flow** — menu, login, connecting/loading, credits, full server browser | 🟡 T56.29 (menu + first-run disclaimer + login + offline connect, real EM-4.2c handshake; first `AppState` menu↔gameplay machine; online transport deferred to T56.31) in PR; T56.30 (loading/MOTD/credits) + T56.31 (server browser) pending | `tasks/56-bl82-phase5-ui-audio-parity-tasks.md` |
