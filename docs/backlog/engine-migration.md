...
| EM-3.11 **[M]** | In-game visual smoke (AO/TAA/fog/anims/perf vs old client) | 🔵 24 rounds so far (PRs #32–#99 + round-24 across rounds; round 19 removed the placeholder-mesh mechanism, round 20 fixed LOD-culling frontier flicker with hysteresis, round 21 fixed a distinct shadow-flicker bug — CSM regen + sprite-wind/shadow desync). **Round 24 re-diagnosed the "constant flicker" as NOT a shadow bug** (Matías's A/B ruled out SSAO/TAA/shadow_cascades): the EM-3.11-FH Phase-C LOD-object proxies (pyramid trees / box+pyramid houses) were drawn with no near-band exclusion, overlapping+z-fighting the real detailed near-terrain — unlike the far-terrain sheet, which cuts a camera-following hole. Fix: near-band fragment `discard` on the shared far-terrain material (+`NotShadowCaster` on zone meshes). Awaiting Matías's live retest. EM-3.11p (diagonal stutter) not formally closed. | `2026-07-09-bl82-em311-findings-log.md` |
