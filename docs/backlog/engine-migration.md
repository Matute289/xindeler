...
| **5** | UI (bevy_ui + widget kit), audio & playable parity | 🔵 **in progress, close to done** — EM-5.1–5.9, 5.11, 5.12, 5.14, 5.15, 5.17, 5.18, 5.19 all ✅ shipped/merged. Only 3 things remain: **EM-5.10 (Audio)** 2 of 5 sub-phases in (foundation + SFX; music/ambience, spatial/muffling, instrument bank still to build); **EM-5.16 (i18n)** the reactive pipeline is proven end-to-end but only 2 of ~15 screens are actually converted to real `.ftl` lookups (§Q6=A locks full coverage as required, not optional); **EM-5.13** the terminal cutover gate (Matías's own full-parity play session + sign-off), blocked until the above two are done. |
