...
# Engine-isolation guard (logic crates must never depend on bevy/wgpu/winit)
...
Distilled from EM-6.1 (PR #203, 2026-07-23): 184 `gitlab/master` commits, 43 hand-resolved conflicts.
The target for every subsequent drill is a **<1-day routine**. Read this list BEFORE Phase 2 and use it
as the post-merge checklist in Phase 4/5.
...
2. **A clean git merge is NOT a green build.** Upstream reworked `common/src/comp/ability.rs` from bundling
   ...
   conflicts**, then our own `bevy/xindeler-sim-bridge` failed to compile against the new signature.
   → Phase 4's `cargo check --workspace` is **mandatory and non-negotiable**, and must cover the `bevy/*`
   crates (the sim-bridge is the most exposed — it is the only legal `specs` consumer under `bevy/`). If
   the fix is non-trivial, invoke `superpowers:writing-plans` before editing, per the Decision Matrix.
...
3. **Required-field additions to core structs break call sites with no conflict.** `humanoid::Body` gaining
   ...
   type upstream touched (`git grep -n "Body {"`), don't assume the conflict list is complete.
...
`xindeler-world`'s economy-sim test fails on clean `development` too (confirmed during EM-6.1). If it is the
ONLY failure and reproduces on a clean checkout, it is not a merge regression — note it and move on.
