...
- The lore canon now ships a machine-readable id index (`assets/lore/index.ron`), loaded via `common::lore::LoreIndex`, with canon-lint tests that keep the index in sync with the design-repo lore tree and block denylisted D&D/Critical Role/Exandria names from leaking into in-world prose (the tests skip when the private lore repo is not checked out).
