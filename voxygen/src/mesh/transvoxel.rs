...
// Each u16 encodes:
//   bits 11-8: first corner index (0-7)
//   bits  7-4: second corner index (0-7)
//   bits  3-0: vertex reuse flags (0 = always emit new vertex)
//
// Source: regularVertexData[256][12] in Transvoxel.cpp (public domain).
//
// TODO (Task 8): Replace the placeholder zeros below with the real table from
//   https://transvoxel.org/Transvoxel.cpp  (array `regularVertexData`).
//   Until then, mesh_transvoxel() produces no interior triangles.
// ---------------------------------------------------------------------------
        ...
        for cy in 0..size.y - 1 {
            for cz in 0..size.z - 1 {
                ...
                for v in 0..vtx_count {
                    ...
                    // Bits 11-8: first corner, bits 7-4: second corner.
...
mod tests {
    ...
    /// This test requires REGULAR_VERTEX_DATA to be populated (Task 8).
    /// Until then it is skipped so the build stays green.
    #[test]
