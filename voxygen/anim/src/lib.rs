...
pub trait Skeleton: Send + Sync + 'static {
    ...
    fn compute_matrices(
        ...
        #[cfg(feature = "use-dyn-lib")]
        {
            ...
            > = // SAFETY: `Self::COMPUTE_FN` names a symbol generated by this
                // crate's `skeleton_impls!` macro with `#[unsafe(export_name)]`
                // in the `voxygen-anim` dylib, compiled from this same
                // workspace by the hot-reload watcher; the symbol's type
                // signature here matches that exported fn item exactly.
...
pub trait Animation {
    ...
    fn update_skeleton(
        ...
        #[cfg(feature = "use-dyn-lib")]
        {
            ...
            > = // SAFETY: `Self::UPDATE_FN` names a symbol generated by this
                // crate's animation macros with `#[unsafe(export_name)]` in
                // the `voxygen-anim` dylib, compiled from this same workspace
                // by the hot-reload watcher; the symbol's type signature here
                // matches that exported fn item exactly.
                unsafe {
