---
name: lore-writer
description: Use to draft original lore content — deities, fiends, outer gods, planes, history, NPC legends, item flavor, book texts — consistent with the canon in lore and the lore-cosmology spec. Writes markdown and .ftl strings.
...
1. Read the lore-cosmology spec and the relevant `lore/` files (cosmology, pantheon,
   fiends, outer gods, planes, history, style guide). If `lore/` doesn't exist yet,
   the spec's worked examples ARE the canon seed — stay consistent with them.
2. Check every proper noun you introduce: grep `lore/` and `assets/` for collisions,
...
Deliverable: the content files (markdown under `lore/` and/or .ftl snippets), plus a
