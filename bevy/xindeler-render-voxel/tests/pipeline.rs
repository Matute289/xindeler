...
// spent five rounds retuning a synchronous placeholder box that used to
// stand in for a chunk's real mesh while it streamed in. Round 19 removed
// the whole mechanism (Minecraft/`xindeler-old` comparison: neither ever
// renders a stand-in for a not-yet-meshed chunk — nothing is drawn there
// until the real mesh lands). These tests replace the old
// EM-3.11h-through-18 placeholder-specific suite and pin the new invariant:
// a key has ZERO entities from the moment it's marked dirty until its real
// mesh finishes and uploads.

/// The core round-19 regression: a key's FIRST ever mark must leave NOTHING
/// indexed/spawned until its real mesh actually lands — never a synchronous
/// stand-in of any kind. This is the direct inverse of the old
/// `first_mark_never_leaves_a_frame_with_nothing_indexed` placeholder test
/// this round deleted.
#[test]
fn first_mark_spawns_nothing_until_the_real_mesh_lands() {
    let mut app = test_app(2, Arc::new(AtomicBool::new(true)));
    ...
    assert!(
        app.world().resource::<ChunkMeshIndex>().get(key).is_none(),
        "round 19: a freshly-marked key must not be indexed until its real mesh lands — no \
         placeholder of any kind"
    );
    assert_eq!(
        terrain_entity_count(&mut app),
        0,
        "round 19: nothing should be spawned at the chunk's footprint before its real mesh exists"
    );

    // The real mesh eventually appears — exactly once, fully formed.
    run_until_complete(&mut app, 1);
    assert_eq!(terrain_entity_count(&mut app), 1);
    assert!(
        app.world()
        ...
        "once indexed, a key always carries its REAL mesh (round 19: no other state exists)"
    );
}

/// `ChunkMeshIndex::has_real_terrain_mesh` must read `false` for the whole
/// window between marking a key dirty and its real mesh landing — the exact
/// signal `xindeler-client::sprite_view` (round 18) and the camera-boom
/// collision fix (round 19) both gate on.
#[test]
fn has_real_terrain_mesh_is_false_until_the_mesh_actually_uploads() {
    let mut app = test_app(1, Arc::new(AtomicBool::new(true)));
    let key = VVec2::new(2, 2);
    ...
    assert!(
        !app.world()
            .resource::<ChunkMeshIndex>()
            .has_real_terrain_mesh(key),
        "still not real immediately after marking — the mesh task hasn't finished yet"
    );
