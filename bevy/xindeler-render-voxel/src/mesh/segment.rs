// LIFT-COPY (BL-82 EM-3.1, Mapper C5-C7) from voxygen/src/mesh/segment.rs @
// 6f9afd978c
//
// Divergences from the original (keep this list honest for the Mapper §E log):
// - `use crate::render::{Mesh, ParticleVertex, SpriteVertex, TerrainVertex,
//   pipelines::FigureSpriteAtlasData}` → portable equivalents from
//   `crate::mesh::{mesh, vertex}` (see vertex.rs for the packed→portable
//   mapping).
// - `use crate::scene::math` → plain `vek` (voxygen's `scene::math` is a
//   re-export of vek's repr_c/repr_simd geometry; `math::Aabb` → `Aabb`).
// - `make_face_texel` texel type: upstream packed `&mut [u8; 4]` → portable
//   `&mut FigureColLight`; same call shape (still built via
//   `TerrainVertex::make_col_light_figure`).
...
use crate::mesh::{
    ...
    mesh::Mesh,
    terrain::FaceKind,
    vertex::{FigureColLight, FigureSpriteAtlasData, ParticleVertex, SpriteVertex, TerrainVertex},
};
use common::{
    ...
    V: BaseVol<Vox = Cell> + ReadVol + SizedVol + 'a,
{
    ...
    greedy.push(GreedyConfig {
        ...
        make_face_texel: |col_light: &mut FigureColLight, vol: &mut V, pos, light, _, _| {
    ...
    V: BaseVol<Vox = Cell> + ReadVol + SizedVol + 'a,
{
    ...
    greedy.push(GreedyConfig {
        ...
        make_face_texel: move |col_light: &mut FigureColLight,
                               flat: &mut _,
                               pos,
                               light,
                               _glow,
