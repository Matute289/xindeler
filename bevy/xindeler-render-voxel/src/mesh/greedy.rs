// LIFT-COPY (BL-82 EM-3.1, Mapper C5-C7) from voxygen/src/mesh/greedy.rs @
// 6f9afd978c
//
// Divergences from the original (keep this list honest for the Mapper §E log):
// - `use crate::render::{Vertex, mesh::Quad, pipelines::AtlasData}` → `use
//   crate::mesh::{mesh::Quad, vertex::{AtlasData, Vertex}}` (portable types,
//   see vertex.rs). Everything below the imports is byte-identical.

use crate::mesh::{
    mesh::Quad,
    vertex::{AtlasData, Vertex},
};
