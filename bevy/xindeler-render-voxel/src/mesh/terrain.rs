// LIFT-COPY (BL-82 EM-3.1, Mapper C5-C7) from voxygen/src/mesh/terrain.rs @
// 6f9afd978c
//
// Divergences from the original (keep this list honest for the Mapper §E log):
// - `use crate::render::{AltIndices, FluidVertex, Mesh, TerrainAtlasData,
//   TerrainVertex, Vertex}` → portable equivalents from `crate::mesh::{mesh,
//   vertex}` (see vertex.rs for the packed→portable mapping).
// - `use crate::scene::terrain::{BlocksOfInterest, DEEP_ALT, SHALLOW_ALT}`: the
//   two constants are lifted inline below (they lived in
//   voxygen/src/scene/terrain/mod.rs:111-114); `BlocksOfInterest` is NOT ported
//   — upstream `generate_mesh` binds it as `_boi` (unused, glow-block search is
//   a TODO there too), so the tuple slot is `()` here. If upstream starts using
//   it, port `scene/terrain/watcher.rs` alongside.
// - `make_face_texel`'s texel type: upstream writes packed `(&mut [u8; 4], &mut
//   u8)`; here it is `(&mut ColLight, &mut u8)` — same call shape, the packing
//   moved into the portable `ColLight` struct.
// - `#![expect(clippy::option_map_unit_fn)]` added file-locally (upstream has
//   it crate-wide in voxygen/src/lib.rs).
// - doc link `[crate::scene::terrain::Terrain::skip_remesh]` → backticked text
//   (no such module here).
// - `#[expect(clippy::needless_lifetimes)]` on `generate_mesh` ('a became
//   elidable only because `_boi` was dropped).
...
use crate::mesh::{
    ...
    mesh::Mesh,
    vertex::{AltIndices, ColLight, FluidVertex, TerrainAtlasData, TerrainVertex, Vertex},
};
use common::{
...
// Lifted from voxygen/src/scene/terrain/mod.rs:111-114 (the mesher's only
// scene-side inputs): terrain slice boundaries for `AltIndices` culling.
...
// NOTE(lift-copy): `'a` became elidable only because the unused
// `&'a BlocksOfInterest` slot was dropped; kept for upstream diffability.
#[expect(clippy::needless_lifetimes)]
pub fn generate_mesh<'a>(
    vol: &'a VolGrid2d<TerrainChunk>,
    // NOTE: `()` replaces upstream's `&'a BlocksOfInterest` (bound as `_boi`,
    // unused — see file header).
    (range, max_texture_size, _boi): (Aabb<i32>, Vec2<u16>, ()),
) -> MeshGen<
    TerrainVertex,
    FluidVertex,
    TerrainVertex,
...
> {
    ...
    greedy.push(GreedyConfig {
        ...
        make_face_texel: |(col_light, kind): (&mut ColLight, &mut u8),
                          data: &mut (),
                          pos,
                          light,
                          glow,
...
/// `crate::scene::terrain::Terrain::skip_remesh` (voxygen).
