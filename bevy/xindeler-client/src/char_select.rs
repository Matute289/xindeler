...
fn rebuild_content(
    ...
    *last_key = Some(key);
    ...
    commands.entity(root).with_children(|parent| match *screen {
        CharSelectScreen::Roster => build_roster(parent, &theme, &fonts, images, &list),
        CharSelectScreen::Wizard => build_wizard_step(parent, &theme, &fonts, images, &state),
    });
}
...
fn build_roster(
    ...
    if list.0.loading {
        ...
        for entry in &list.0.characters {
            ...
            spawn_roster_slot(parent, theme, images, Color::WHITE, |row| {
                ...
                spawn_action_button(row, theme, fonts, "Enter", CharAction::SelectCharacter(*id));
                spawn_action_button(
                    row,
                    theme,
                    fonts,
    ...
    // legacy `ui.rs`'s green-tinted create slot.
    spawn_roster_slot(parent, theme, images, Color::srgb(0.55, 1.0, 0.5), |row| {
        spawn_action_button(row, theme, fonts, "+ Create New", CharAction::OpenWizard);
    });
}
