...
//!
//! ## `DimensionId` — folded into the real, registry-backed identifier
//! `RegionKey`/`ClientVisibleRegions` originally keyed on a deliberately
//! scoped placeholder (`DimensionId(u32)`, always `0` — no dimension concept
//! existed anywhere in this codebase when this task was written, confirmed by
//! `specs/2026-07-10-bl82-phase4-remaining-plan.md` §0.3). That placeholder
//! was scoped to this module only, specifically so folding in the REAL
//! identifier (once EM-4.5's sibling branch landed) would be "a rename/
//! replace of one small type, not a redesign" — see [`crate::dimension_id`]
//! for the now-real `DimensionId` this module uses, and that module's own
//! doc comment for why it lives in `xindeler-protocol` rather than
//! `xindeler-dimensions`.
...
use crate::dimension_id::DimensionId;
